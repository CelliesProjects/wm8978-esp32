...
{
    uint8_t res = Write_Reg(0, 0); // 软复位WM8978
    if (res)
...
{
    uint16_t regval = Read_Reg(2); // 读取R2
    if (micen)
    ...
    if (lineinen)
        setLINEINgain(5); // LINE IN 0dB增益
    else
        setLINEINgain(0); // 关闭LINE IN
    if (auxen)
        setAUXgain(7); // AUX 6dB增益
    else
        setAUXgain(0); // 关闭AUX输入
}
// WM8978 输出配置
...
{
    gain &= 0X07;
    uint16_t regval = Read_Reg(47);    // 读取R47
    regval &= ~(7 << 4);               // 清除原来的设置
    Write_Reg(47, regval | gain << 4); // 设置R47
    regval = Read_Reg(48);             // 读取R48
    regval &= ~(7 << 4);               // 清除原来的设置
